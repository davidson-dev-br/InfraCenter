...
>(
  (
    ...
    // Ler o estado inicial do cookie
    const getInitialPinnedState = () => {
      ...
      return cookieValue ? cookieValue === 'true' : defaultPinned;
    };
    ...
    const state = (isPinned || isHovered) ? "expanded" : "collapsed";
    ...
    const contextValue = React.useMemo<SidebarContext>(
      ...
      [state, isPinned, setIsPinned, isMobile, openMobile, setOpenMobile, toggleSidebar, isHovered]
    )
...
>(
  (
    ...
    const { isMobile, isPinned, state, openMobile, setOpenMobile, setIsHovered } = useSidebar()
...
>(({ className, onClick, ...props }, ref) => {
  const { toggleSidebar, isPinned, state } = useSidebar()
    ...
    <Tooltip>
        <TooltipTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="icon"
              className={cn("h-7 w-7 transition-opacity", state === "collapsed" && "opacity-0 pointer-events-none", className)}
              onClick={(event) => {
                onClick?.(event)
                toggleSidebar()
              }}
