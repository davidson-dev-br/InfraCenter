...
import { extractEquipmentDetails } from '@/ai/flows/extract-equipment-details-flow';
...
export function IAEquipamentoDialog({ children }: IAEquipamentoDialogProps) {
  ...
  const [initialFormData, setInitialFormData] = useState<Partial<Equipment> | null>(null);
  ...
  const handleAnalyze = async () => {
    ...
    setStep('loading');
    try {
      const result = await extractEquipmentDetails({ photoDataUri: imageDataUri });
      const newEquipmentData: Partial<Equipment> = {
      ...
      setInitialFormData(newEquipmentData);
      setIsFormOpen(true);
      setIsIaOpen(false);
    } catch (error) {
    ...
    <>
      <Dialog open={isIaOpen} onOpenChange={setIsIaOpen}>
        ...
        <DialogContent onInteractOutside={(e) => { if (step === 'loading') e.preventDefault(); }}>
          <DialogHeader>
            ...
            <DialogDescription>
              {step === 'select' && 'Escolha uma fonte para a imagem do equipamento.'}
              {step === 'camera' && 'Posicione a câmera para capturar os detalhes do equipamento.'}
            ...
            {(step === 'preview' || step === 'loading') && imageDataUri && (
                <div className="space-y-4">
                    <div className="relative border-2 border-dashed rounded-md aspect-video">
                        <img src={imageDataUri} alt="Equipment preview" className="object-contain w-full h-full rounded-md" />
      ...
      <Dialog open={isLargePreviewOpen} onOpenChange={setIsLargePreviewOpen}>
          <DialogContent className="max-w-4xl">
              ...
              <div className="p-4">
                  <img src={imageDataUri || ''} alt="Large equipment preview" className="w-full h-auto max-h-[70vh] object-contain rounded-md" />
              </div>
              <DialogFooter>
                  <a href={imageDataUri || ''} download={`equipment-${Date.now()}.jpg`} className="w-full sm:w-auto">
