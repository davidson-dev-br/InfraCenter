...
export function FloorPlan() {
    ...
    const roomWidthM = selectedRoom?.width || 12; // 12m default
    const roomLengthM = selectedRoom?.length || 12; // 12m default
    ...
    const checkCollision = useCallback((testItem: PlacedItem, allItems: PlacedItem[]) => {
        ...
        if (testItem.x + testDim.width > GRID_COLS || testItem.y + testDim.length > GRID_ROWS) {
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            ...
            <div ref={floorPlanRef} className="flex-grow p-4 overflow-auto border rounded-lg bg-card" onClick={() => setSelectedItemId(null)}>
                <div className="relative transition-transform duration-75" style={{ transform: `scale(${zoom})`, transformOrigin: 'top left', width: `${(GRID_COLS * CELL_SIZE) + 40}px`, height: `${(GRID_ROWS * CELL_SIZE) + 30}px` }}>
